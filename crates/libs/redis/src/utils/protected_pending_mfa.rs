...
impl ProtectedPendingMfa {
    ...
    const CONTEXT: &str = "kestrel 2026-05-28 00:11:13 pending mfa protected payload";
    ...
    pub fn decrypt(mut self, temp_token: &str) -> Result<PendingMfa, ProtectedPendingMfaError> {
        let protection_key = derive_key(Self::CONTEXT, temp_token.as_bytes());
        let cipher = XChaCha20Poly1305::new(&protection_key.into());

        let mut protected_payload = base32_decode(&self.pending_mfa.protected_payload)
        ...
        self.pending_mfa.protected_payload = String::from_utf8(protected_payload)
            .map_err(|_| ProtectedPendingMfaError::Decryption)?;
